...
- SkillLens 检查
- test-prompts.json 存在性检查（位于 `artifacts/<skill>/`）；缺失时自动生成 3 个**占位模板** prompts 并给出警告，正式版应由 Agent 撰写
...
```bash
# 跨 skill 汇总 scorecard
evaluate-skill --all --skills-dir ./skills --output reports/SKILL_SCORECARD.md

# 单个 skill 的详细报告
evaluate-skill skills/my-skill --output artifacts/my-skill/scorecard.md --detailed
```
