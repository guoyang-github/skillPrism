...
| 渐进测试便捷入口 | "跑渐进测试"（便捷命令） | `skill-gradual --skill <skill> --registry benchmarks/<skill>/registry.yaml`（`test-skill --mode gradual` 的封装） |
...
| `--task <task>` | task id | 可选；不传则跑 registry 内全部 benchmark |
...
| `--no-stop-on-regression` | 回滚后继续自动编辑（默认：遇到回滚即停止） | 探索式自动优化 |
...
skill-pipeline --intent "run full quality pipeline" \
    --skills-dir ./skills \
    --benchmark-registry benchmarks/my-skill/registry.yaml \
    --output reports/SKILL_QUALITY_REPORT.md --run-smoke
```
...
| `--no-stop-on-regression` | 关闭"回归即失败"（默认：benchmark 退化即失败） | 只记录不 fail |
...
# 含 benchmark 的完整 CI（默认 benchmark 退化即失败）
skill-ci --skill my-skill \
    --run-benchmark --code sample_skill_code.py \
    --baseline baselines/my-skill.yaml

# 关闭回归即失败（只记录，不 fail）
skill-ci --skill my-skill --no-stop-on-regression
```
...
- 自然语言交互的完整话术：本表上文「自然语言 → CLI 命令」与 `skills/skill-prism/SKILL.md`。
- 架构设计：[系统总览](../reference/overview.md)（含架构设计、模块边界与数据流）。

---

## 附录：Agent 汇报模板

Agent 执行完评估或完整流水线后，应按以下格式向用户汇报（与 `skills/skill-prism/SKILL.md` 的汇报约定一致）。
