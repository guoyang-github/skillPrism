...
## 0.0 准备一个 Skill（模板结构 + frontmatter）

如果你还没有 skill，可以用仓库自带模板快速创建（模板见仓库 `templates/` 目录）：

```bash
mkdir -p skills/csv-summary-skill
cp -r templates/analysis/* skills/csv-summary-skill/
```

模板目录结构：

```text
skills/csv-summary-skill/
├── SKILL.md              # 唯一必需：教 Agent/用户怎么用的说明书
├── examples/             # 最小可运行示例（强烈建议）
│   └── minimal_example.py
├── requirements.txt      # 依赖清单（强烈建议）
└── tests/
    └── test_smoke.py
```

`SKILL.md` 的 frontmatter 逐字段说明：

| 字段 | 作用 |
|---|---|
| `name` | skill 名（kebab-case，与目录名一致） |
| `description` | 一两句话说明用途、何时使用、产出什么；Agent 据此决定是否调用，要具体、关键词丰富 |
| `tool_type` | 工具类型：`python` / `r` / `cli` / `api` / `document` |
| `primary_tool` | 主要工具/库（如 `pandas`、`scanpy`） |
| `languages` | 语言列表 |
| `keywords` | 关键词列表，用于检索与匹配 |
...
> **registry 从哪来？** 从 0.2 起用到的 `benchmarks/csv-summary-skill/registry.yaml` 不是引擎自动生成的：需要由 Agent（或你）先用 `build-skill-test` 逐条注册 benchmark——task spec、输入数据、金标准、metrics 声明都在这一步落进 registry。完整构建流程见 [构建你的第一个 Benchmark](../tutorial/04-building-your-first-benchmark.md)；Agent 场景下，Agent 会按 `skills/skill-prism/references/BUILD_BENCHMARK.md` 的引导流程（确认任务契约 → 准备数据 → 注册 → 验证）带你完成。
