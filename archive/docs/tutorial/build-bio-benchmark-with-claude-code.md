...
安装后会得到七个命令：`evaluate-skill`、`test-skill`、`build-skill-test`、`improve-skill`、
`skill-pipeline`、`skill-ci`、`skill-gradual`。验证：
...
跑渐进（level 0 → level 1，失败即停、通过后自动 ratchet 该级基线）：
...
!!! info "`--mode gradual` 与 `--suite <name>` 的区别（统一口径）"
    - `--mode gradual`：渐进**模式**，带 stage 逐级放行、fail-fast（首失败即停）、
      ratchet（通过后更新该级基线到 `artifacts/<skill>/ci/gradual/.baselines/`）语义，**推荐**用来跑渐进测试。
    - `--suite <name>`：只是按 registry 里定义的 suite **过滤**要跑哪些 benchmark，
      不含逐级/ratchet 语义。两者可组合（如 `--mode gradual --suite gradual`）。
...
test-skill --skill $SKILL \
  ...
  --max-level 1 \
  --code sample_skill_code.py \
  --output artifacts/$SKILL/results.yaml
```

> 结果快照属于生成物，约定放在 `artifacts/<skill>/` 下（不进 `benchmarks/<skill>/` 的
> 核心五件：registry.yaml、tasks/、data/、expected/、可选 metrics.py）。
> 这份 yaml 由用户自己管理——后续改动 skill 后再跑一次，diff 这份快照即可看出回归/改进；
> 接 CI 时可直接作为 `skill-ci --baseline` 的对比基线。
...
6. 写 sample_skill_code.py（确定性的簇→细胞类型 mock 映射），并用
   test-skill --mode gradual --max-level 1 --code sample_skill_code.py 跑通，给我 PASS/FAIL 结果。
```
...
Claude：
  ...
  7. 调用 test-skill --mode gradual --max-level 1 --code ... 验证，汇报结果
```
...
第 4 轮：写 sample_skill_code.py 并跑 test-skill --mode gradual --max-level 1，汇报结果。
```
...
| 「把当前结果存成快照」 | `test-skill ... --output artifacts/<skill>/results.yaml`（结果 yaml 由用户管理，不进 `benchmarks/<skill>/` 核心五件） |
...
- [ ] `test-skill --mode gradual --max-level 1 --code sample_skill_code.py` 全部 PASS（level 1 的 ari/accuracy 有数值）。
