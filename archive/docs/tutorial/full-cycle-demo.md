...
Test prompts: 3 prompt(s) ready for effect verification
  → test-prompts.json: artifacts/data-summary/test-prompts.json
```
...
`evaluate-skill` 总会附带 test-prompts 检查段：读取（缺失时自动生成模板）
`artifacts/data-summary/test-prompts.json`，用于后续效果验证。
...
**Agent**：我把 `data/counts.csv` 复制进 `benchmarks/data-summary/data/`，
写 task spec `benchmarks/data-summary/tasks/table.yaml`，
再用 `build-skill-test` 注册一个 benchmark：期望输出行数 ≥ 2。金标准由我现场生成。

▶ `build-skill-test --id counts_summary --name "Counts Summary" --skill data-summary --task table --task-spec tasks/table.yaml --input data/counts.csv --expected-path expected/counts_summary.csv --metric row_count:min:2 --registry benchmarks/data-summary/registry.yaml`

> benchmark 构建的完整权威教程（目录结构、task spec、私有 metric、渐进注册）见
...
> **三种 "baseline" 别混淆**：
> - `improve-skill` 的 baseline = `artifacts/<skill>/baseline/baseline.json`，引擎自动维护
>   （原子写 + `.bak` 回退，含 `historical_best_score`），是 keep/revert 的对比基准；
> - `skill-ci --baseline <path>` = **用户自己指定**的对比结果 yaml（如上次 `test-skill --output` 的产物），
>   用于 CI 回归对比；
> - 渐进测试的逐级 baseline = `artifacts/<skill>/ci/gradual/.baselines/<skill>/gradual_baseline_level<N>.yaml`
>   （见 §8），由 gradual 流水线自动 ratchet。
...
Edit reverted. Stopping auto-edit loop.
Pass --no-stop-on-regression to continue after a reverted edit.
```
...
> 这里的 `--baseline baselines/data-summary.yaml` 是用户自管的对比 yaml（区分见 §4 的
> 「三种 baseline」）；`improve-skill` 的 ratchet 真相源才是
> `artifacts/<skill>/baseline/baseline.json`（原子写 + flock）。
...
▶ `evaluate-skill --all --skills-dir ./skills --output reports/SKILL_SCORECARD.md --output-history artifacts/skill_history.jsonl --run-smoke --verbose`

或者一站式流水线（评估 + benchmark + 找最差 skill + 给下一步优化命令）：

▶ `skill-pipeline --intent "run full quality pipeline" --skills-dir ./skills --benchmark-registry benchmarks/data-summary/registry.yaml --output reports/SKILL_QUALITY_REPORT.md --run-smoke`

```
Rubric scorecard → reports/SKILL_QUALITY_REPORT.md
...
| "评估所有 skills" | `evaluate-skill --all --skills-dir ./skills --output reports/SKILL_SCORECARD.md` | 否 |
