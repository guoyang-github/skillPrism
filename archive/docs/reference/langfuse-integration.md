# skillPrism × Langfuse 集成方案（设计提案）

> 将 skillPrism 的「Skill 评估 + 优化」能力嫁接到 Langfuse 的可观测/实验体系上，
> 形成团队级、跨机器、可追溯的工程化评估优化闭环。
>
> 状态：**设计提案（Design Proposal），尚未实施**。实施前需确认部署形态（自建 / Cloud）与数据合规边界。
...
| 风险 | 缓解 |
|---|---|
| 引擎确定性被观测层污染 | Backend 只读采集，不回写引擎结果；noop 默认；全套测试在 noop 路径行为不变 |
...
2. 未设该环境变量时，引擎输出与本轮改造后完全一致（测试全绿）。
...
- **原子状态 + flock** → Langfuse 成为真相源后，本地状态降级为缓存仍保持完整。
- **`_run_auto_edit_rounds` locked wrapper** → trace 注入点清晰（一个外层函数）。
- **`_judge_candidate_unlocked` / `_run_bloat_gate`** → 决策点已结构化，
  `score(decision)` 注入位置明确。
- **regression 方向感知** → metric score 的 `:lower_better` 后缀有现成依据。
- **outlier 日志** → 桥接 Langfuse event 的现成 hook。
- **config schema** → 新增 `llm_judge.backend` 配置项自动被校验。
- **静默 except 改日志** → 观测层降级路径的失败可见。
