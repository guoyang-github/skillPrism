...
| **Table expected** | ⚠️ 半自动 | `--generate-expected` 复制输入；真正统计量需脚本计算 | `expected.path` 指向 CSV |
| **Clustering 输入数据** | ✅ 可以 | `scanpy.datasets` builtin、合成脚本（`sc.datasets.blobs`） | `dataset.type: builtin` 或 `local` |
| **Clustering expected (adata.h5ad)** | ❌ 需参考流程 | 用固定参数跑 scanpy 聚类流程生成金标准 | `expected.path` 指向 h5ad |
| **Document prompt** | ❌ 手动 | 人工设计评估 prompt | `dataset.source` 指向 txt |
| **Document expected (best_skill.md)** | ❌ 手动 | 人工撰写或精选的“最佳答案” | `expected.path` 指向 md |
...
| **Level 0 边界测试输入** | ⚠️ 半自动 | `skillprism/testing/boundary.py` 可按任务生成空文件、单列、0 cell 等边界输入，但需任务插件**显式调用** `run_task_boundary_tests`；benchmark 主路径（`run_single_benchmark`）不会自动接入 | 由调用方把报告写入 `_boundary_report` |
...
2. （可选）编写私有扩展
   ├── metrics.py    ← 放在 registry 同级目录，用 @metric("id") 注册私有 metric
   └── plugins       ← 复杂任务用任务插件接管执行（见扩展文档）
...
注意：引擎**不会**加载 `benchmarks/<skill>/runner.py` 之类的自定义 runner 文件；执行路径只有「任务插件」或「task spec + `--code`/Agent/results 模式」（见 `skillprism/benchmark/runner.py` 的 `run_single_benchmark`）。
...
build-skill-test \
  ...
  --task table \
  --input data/sales/input/sales.csv \
  --expected-path expected/sales/sales_summary.csv \
  --metric row_count:min:3 \
  --metric revenue_sum:min:500 \  # revenue_sum 为私有 metric，需在 registry 同级 metrics.py 用 @metric 注册
  --generate-expected \
...
4. **边界测试依赖 skill 代码**：level 0 的边界输入由 `skillprism/testing/boundary.py` 生成，但只有当任务插件显式调用 `run_task_boundary_tests` 时才会执行；且边界测试能不能通过取决于 Skill 代码的健壮性。
