...
每条 prompt 必须是**具体输入 + 具体可验证的期望**，禁止 "Use this skill to ..." 这类元指令写法（规范见 `skills/skill-prism/references/PROMPTS_VERIFICATION.md` Step 1）：
...
`evaluate-skill` 默认会在 `test-prompts.json` 缺失时自动生成 3 个 prompts（happy path、ambiguous、boundary），写入 `artifacts/<skill>/test-prompts.json`（可用 `--prompts-dir` 覆盖）。**自动生成的是通用占位模板**，引擎会给出 ⚠️ 占位符警告；正式版应由 Agent 按协议撰写（规范见 `skills/skill-prism/references/PROMPTS_VERIFICATION.md`）。
