...
> **本文是 skillPrism 中 benchmark metric 主题的唯一权威源**；其他文档涉及 metric 时只给链接，不重复清单。
>
> 本文回答三个常见问题：
...
> 注意：`_all_pass` 是结果里的**聚合字段**（所有 metric 是否全部通过），不是可注册的 metric id，不能写进 `metrics` 列表。引用了未注册的 metric id 时，`GenericEvaluator` 会把它记为 `None` 并直接判 FAIL——没有“跳过并提示”的逻辑。
...
下面两张表与 `skillprism/benchmark/metrics.py` 中的 `@metric(...)` 注册表逐一对应（共 20 个 id）；引用表外 id 会直接判 FAIL。
...
如果 expected 文档较长、需要语义级对比，可以按 §1.1 的方式写私有 metric（自行接入嵌入模型或 ROUGE/BERTScore 等库）；
