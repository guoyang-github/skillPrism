> Rubric 规则增强把 darwin-skill / SkillLens 中验证过的质量信号（模糊词、失败模式编码、检查点标记等）变成可执行的默认检查。
...
目前规则增强是默认行为，无法通过 CLI 单独禁用。如果你认为某项检查过于严格，可以：

1. 在 `SKILL.md` 中避免触发该检查的措辞；
2. 或通过 `--no-generate-prompts` 等参数关闭相关联动功能。
