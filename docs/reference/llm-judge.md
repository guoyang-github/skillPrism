...
3. **如果未配置**：Agent 自己调用 LLM 生成 `.skillprism_llm_judgments.json`，然后调用 `evaluate-skill --llm-judgments <file>`。
...
Agent 直接调用 LLM，生成 `.skillprism_llm_judgments.json`，然后让引擎消费。此方式不需要配置 `SKILLPRISM_LLM_JUDGE_COMMAND`。
...
6. 写入 .skillprism_llm_judgments.json
7. 调用 evaluate-skill --llm-judgments .skillprism_llm_judgments.json
```
