...
- **只需要复制 `SKILL.md` 这一个文件**。`skills/skill-prism/` 目录下的 `test-prompts.json`、`.skillprism_baseline/`、`.skillprism_history.jsonl` 等是 skillPrism 引擎运行后产生或读取的数据，不需要放到 `.claude/skills/` 里。
- 如果 skill 文件很大，会占用一部分上下文窗口。skill-prism SKILL.md 约 300 行，通常可接受。
- 同样地，具体业务 Skill 也可以放到 `.claude/skills/`，例如 `.claude/skills/my-skill.md`；但业务 Skill 的工作目录仍建议保留在 `skills/my-skill/`，供 `evaluate-skill` / `improve-skill` 读写历史数据。
