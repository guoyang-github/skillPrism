...
| 找出最差的并准备优化 | "哪个 skill 最差？帮我准备优化" | `improve skills` |
...
| 意图 | 说明 |
|---|---|
| `evaluate all skills` / `score all skills` / `评估所有 skills` | 只跑 Rubric 评估 |
| `run benchmarks` / `跑 benchmark` | 只跑测试并对比基线 |
| `run full quality pipeline` / `跑完整流水线` | 评估 + 测试 + 找最差 skill |
| `improve skills` / `优化 skills` | 完整流水线 + 为最差 skill 记录 baseline |
| `run gradual pipeline` / `渐进测试` | 跑 level 0→3 渐进测试 |
